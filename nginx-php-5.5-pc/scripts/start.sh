...
if [ ! -z "$AWS_ACCESS_KEY" ]; then
 sed -i "s/REPLACE_WITH_AWS_ACCESS_KEY/${AWS_ACCESS_KEY}/g" .aws/credentials
fi

if [ ! -z "$AWS_SECRET_KEY" ]; then
 sed -i "s/REPLACE_WITH_AWS_SECRET_KEY/${AWS_SECRET_KEY}/g" .aws/credentials
fi

if [ ! -z "$AWS_REGION" ]; then
 sed -i "s/REPLACE_WITH_AWS_REGION/${AWS_REGION}/g" .aws/config
else
 sed -i "s/REPLACE_WITH_AWS_REGION/us-east-1/g" .aws/config
fi
